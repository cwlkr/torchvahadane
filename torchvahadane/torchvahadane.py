...
class TorchVahadaneNormalizer():
    ...
    def fit(self, slide_name, dataloader, p, div=3):
        ...
        for n, (data, coor, size) in tqdm(enumerate(dataloader)):
            ...
            try:
                ...
                pass
        stain_mat = torch.from_numpy(np.median(np.array(stain_mats), axis=0)).to(self.device)
        max_c = torch.from_numpy(np.median(np.array(max_cs), axis=0)).to(self.device)
    ...
    def transform(self, slide_name, dataloader, p, new_slide):
        ...
        vi.tiffsave(save_path, tile=True, compression='jpeg', bigtiff=True, pyramid=True) # type: ignore

        if cfg.get_thumb:
            quick_check_path = os.path.join(cfg.source, 'quick_check', slide_name.replace('.' + fmt, '_thumbn_cn.png'))
            thumbn = Image.fromarray(new_slide)
            thumbn.thumbnail((4096, 4096), Image.LANCZOS)
            thumbn = np.array(thumbn)[:, :, :3].astype(np.uint8)
            cv2.imwrite(quick_check_path, cv2.cvtColor(thumbn, cv2.COLOR_BGR2RGB))
